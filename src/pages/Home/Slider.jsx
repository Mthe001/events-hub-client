...
const SwiperSlider = () => {
    ...
    const { data: events = [], isLoading } = useQuery({
        queryKey: ["events"],
        queryFn: async () => {
            const { data } = await axiosPublic.get(`/all-events`);
            return data;
        },
    });
    ...
    return (
        isLoading ?
            ...
            :
            <div className="relative w-full max-w-screen-xl mt-10 mx-auto">
                <TextAnimate className="mb-10 lg:text-5xl md:text-4xl text-3xl text-center font-semibold dark:text-blue-600 text-black" animation="blurInUp" by="character">
                    Top Events by : NEWS 240
                </TextAnimate>
                <Swiper
                    modules={[Navigation, Pagination, Autoplay, EffectCoverflow]}
                    effect="coverflow"
